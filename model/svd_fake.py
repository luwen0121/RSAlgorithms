...
class SVD(MF):
    ...
    def train_model(self, k):
        ...
        while iteration < self.config.maxIter:
            ...
            self.loss += self.config.lambdaP * (self.P * self.P).sum() + self.config.lambdaQ * (self.Q * self.Q).sum() \
                         + self.config.lambdaB * (
                                 (self.Bu * self.Bu).sum() + (self.Bi * self.Bi).sum()) 
            iteration += 1
    ...
    def predict(self, u, i):
        if self.rg.containsUser(u) and self.rg.containsItem(i):
            ...
            return self.Q[i].dot(self.P[u]) + self.rg.globalMean + self.Bi[i] + self.Bu[u]
        else:
